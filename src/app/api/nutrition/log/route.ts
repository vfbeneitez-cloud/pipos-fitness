...
import { createNutritionLog } from "@/src/server/api/nutrition/log";
...
export async function POST(req: Request) {
  return withSensitiveRoute(req, async () => {
    ...
    if (authResult instanceof NextResponse) {
      trackEvent("nutrition_log_post_unauthorized", { status: 401 }, { sentry: true });
      return authResult;
    }
    ...
    try {
      ...
      trackEvent("nutrition_log_post_badRequest", { status: 400 });
      return NextResponse.json(badRequestBody("INVALID_JSON"), { status: 400 });
    }
    const result = await createNutritionLog(body, userId);
    ...
    trackEvent("nutrition_log_post_success", { status: 200 });
