...
vi.mock("@/src/server/plan/validateWeeklyPlan", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@/src/server/plan/validateWeeklyPlan")>();
  return {
    ...actual,
    validateTrainingBeforePersist: vi.fn(actual.validateTrainingBeforePersist),
    validateNutritionBeforePersist: vi.fn(actual.validateNutritionBeforePersist),
  };
});
...
describe("weekly plan v0", () => {
  ...
  it("returns null for non-existent plan", async () => {
    // Use a week no other test creates to avoid pollution (e.g. training/log ownership test)
    const getRes = await getWeeklyPlan(
      `http://localhost/api/weekly-plan?weekStart=2030-01-06`,
      TEST_USER_ID,
    );
  ...
  it("returns 400 and INVALID_TRAINING_PLAN when training validation fails", async () => {
    await prisma.userProfile.upsert({
      where: { userId: TEST_USER_ID },
    ...
    vi.mocked(validateTrainingBeforePersist).mockReturnValueOnce({
      ok: false,
    ...
    expectApiError(result, { status: 400, code: "INVALID_TRAINING_PLAN" });
  });
