...
export async function getWeeklyAdherenceInsightsHandler(url: string, userId: string) {
  const u = new URL(url);
  ...
  const planInput = {
    ...
    mealsPerDay: nutritionPlan.mealsPerDay ?? 3,
    cookingTime: nutritionPlan.cookingTime,
  };
