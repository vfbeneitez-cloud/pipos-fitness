...
- **Errores y códigos**:
  - inputs inválidos → **4xx** (normalmente `400 Bad Request` / `404 Not Found` / `422 Unprocessable Entity`);
  - errores inesperados (bugs, fallos DB no anticipados) → **5xx**;
  - formato de error JSON:
    - `{ "error": "ERROR_CODE", "details"?: any }`
    - `details` puede incluir errores de validación, pero nunca secretos ni PII sensible.
...
- Al añadir/modificar un endpoint:
  - verificar que:
    ...
    - los errores de validación devuelven `400` o `4xx` apropiado con `{ error, details }`,
